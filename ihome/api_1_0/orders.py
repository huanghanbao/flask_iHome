...
@login_required
def updata_order_status(order_id):
    """
    接单或者拒单操作
    ...
    try:
        ...
        if order.house.user_id != user_id:
            return jsonify(errno=RET.DATAERR, errmsg="不是房屋房东")
    except Exception as e:
        current_app.logger.error(e)
        return jsonify(errno=RET.DBERR, errmsg="查询订单失败")
...
# 如果role == lodger，以方可身份查询自己预定别人房屋的订单信息
# 如果role == landlord，以房东的身份查询别人预定自己房屋的订单信息
@api.route("/orders")
@login_required
def get_order_list():
    """
    获取用户的订单信息：
    1、获取用户的角色role
        1.1 如果role == lodger，以房客身份查询自己预定别人房屋的订单信息
        1.2 如果role == landlord，以房东的身份查询别人预定自己房屋的订单信息
    2、组织数据，返回应答
    """
    # 1、获取用户的角色role
    ...
    try:
        if role == "lodger":
            # 1.1如果role == lodger，以房客身份查询自己预定别人房屋的订单信息
            orders = Order.query.filter(Order.user_id == user_id).order_by(Order.create_time.desc()).all()
        else:
            # 1.2如果role == landlord，以房东的身份查询别人预定自己房屋的订单信息
            # 获取房东所有的房屋信息
